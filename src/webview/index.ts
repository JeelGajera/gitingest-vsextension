...
export function getResultsContent(
    ...
    const content = `<div class="shadow-wrapper"><div class="content-wrapper">${filterPanel}<div class="grid"><div>${summarySection}${buttonGroup}</div>${treeSection}</div>${contentSection}</div></div><script>${resultsScript(ingestedPathTrimmed)}</script>`;
    return createHtmlDocument(content, getBaseStyles(THEME));
}
...
function saveToFile() { vscode.postMessage({ command: 'saveToFile', data: { summary: raw('gi-summary'), tree: raw('gi-tree'), content: raw('gi-content') } }); }
`;
}
